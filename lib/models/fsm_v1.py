...
class FSnet_v1(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=True):
        ...
        self.FSM3 = self.FSM(3, highres_planes, planes*4, highres_planes)
        self.FSM4 = self.FSM(4, highres_planes, planes*8, highres_planes)
...
def FSnet_imagenet(cfg, pretrained=False):
    model = FSnet_v1(BasicBlock, [2, 2, 2, 2], num_classes=19, planes=32, spp_planes=128, head_planes=64, augment=True)
    if pretrained:
...
def get_seg_model(cfg, **kwargs):
    ...
    model = FSnet_imagenet(cfg, pretrained=True)
    return model


if __name__ == '__main__':
    x = torch.rand(4, 3, 800, 800)
    net = FSnet_imagenet(pretrained=True)
    y = net(x)
    print(y.shape)