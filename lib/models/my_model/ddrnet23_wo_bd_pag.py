...
#boundary branch
        self.layer4_b = self._make_single_layer(Bottleneck, planes * 2, planes * 2, 1)
        ...
        if self.augment:
            ...
            self.seghead_boundary = segmenthead(planes * 4, planes, 1, scale_factor=8)
    ...
    def forward(self, x):
        ...
        x_ = self.pag3(x_, self.compression3(x)) # shape = [batch, planes * 2, 1/8, 1/8] (detail3)
        ...
        x_ = self.pag4(x_, self.compression4(x)) #shape = [batch, planes * 2, 1/8, 1/8] (detail4)
        x_b = self.layer4_b(self.relu(x_)) #shape = [batch, planes * 2, 1/8, 1/8] (boundary4)
        #boundary layers
        ...
        if self.augment: 
            ...
            x_boundary_head = self.seghead_boundary(temp_boundary) 

            return [x_detail_head, x_, x_boundary_head]
        else:
            return x_      
...
if __name__ == '__main__':
    x = torch.rand(3, 3, 1024, 2048)
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=19, planes=32, spp_planes=128, head_planes=64, augment=True)
    y = model(x)
    #modelsummary(model, x)
